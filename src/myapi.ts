...
export const myapi = {
	communities: {
		get: {
			many: async () => {
				const data = await myfetch(`${BASE_URL}/communities`)
					.GET()
					.json();
				return validationService.validateApi.communities.get.many(data);
			},
			one: async (cid: string | number) => {
				const data = await myfetch(`${BASE_URL}/communitites/${cid}`)
					.GET()
					.json();
		...
		create: async (uid: string | number, data: unknown) => {
			const res = await myfetch(`${BASE_URL}/users/${uid}/communities`)
				.POST(data)
				.json();
			return res;
		},
	...
	posts: {
		get: {
			many: async () => {
				const data = await myfetch(`${BASE_URL}/posts`).GET().json();
	...
	comments: {
		get: {
			many: async (pid: string | number) => {
				const data = await myfetch(`${BASE_URL}/posts/${pid}/comments`)
					.GET()
					.json();
